...
/**
 * Tests <code>*ContentEE</code> {@link Closeable} content model interfaces,
...
public class ContentModel_cTest {
  ...
  /**
   * Gets the set of all <code>*ContentEE_c</code> {@link Closeable} content model interfaces.
   */
  ...
  static void testContentModels(Class<? extends ContentEE_c> clazz, Class<? extends ContentEE_c>... expected) {
    InheritanceTestHelper.testInterfaces(
        ContentEE_c.class,
        iface -> iface.getSimpleName().endsWith("ContentEE_c"),
        getAllContentModels(),
        clazz,
        expected
    );
